            ...
            for (auto i : fptr->instructions){
                ...
                if (label_cast){
                    ...
                    if (one_succ_branch){
                        ...
                        Block* successorBlock = fptr->blockNameToPointer[one_succ_branch->label->print()];
                        block->successors.push_back(successorBlock);
                    } else if (two_succ_branch){
                        auto block = fptr->blockNameToPointer[curr_label];
                        Block* trueSuccessorBlock = fptr->blockNameToPointer[two_succ_branch->labelTrue->print()];
                        Block* falseSuccessorBlock = fptr->blockNameToPointer[two_succ_branch->labelFalse->print()];
