...
namespace L1 {
    ...
    incdec_instruction::incdec_instruction(Item *reg, String method) : reg(reg), method(method) {}
    ...
    label_Instruction::label_Instruction(String label) : label(label) {}

    // goto_label_instruction Constructor
    goto_label_instruction::goto_label_instruction(String method, String label) : label_Instruction(label), method(method) {}

    // Call_Instruction Constructor
    Call_Instruction::Call_Instruction(String method) : method(method) {}

    // Call_tenserr_Instruction Constructor
    Call_tenserr_Instruction::Call_tenserr_Instruction(Integer F) : F(F) {
    }
    ...
    Memory_assignment::Memory_assignment(Item *dst, String method, Item *x, Item *M) : dst(dst), method(method), x(x), M(M) {}

    // Memory_arithmetic Constructor
    Memory_arithmetic::Memory_arithmetic(Item *dst, String method, Item *x, String instruction, Item *M) 
    ...
    cjump_cmp_Instruction::cjump_cmp_Instruction(Item *t1, String method, Item *t2, String label) : t1(t1), method(method), t2(t2), label(label) {}
    ...
    w_increment_decrement::w_increment_decrement(RegisterID r, String symbol) 
        : r(r), symbol(symbol) {
    }

    w_atreg_assignment::w_atreg_assignment(RegisterID r1, RegisterID r2, RegisterID r3, Integer E) 
